...
BAUD_RATE = 115200
WINDOW_DURATION = 4.0 
GAP_DURATION = 4.0     # 4s static gap after detection
CONFIDENCE_THRESHOLD = 0.40 
SAMPLE_FEQ = 100.0
MADGWICK_BETA = 0.1

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_PATH = os.path.join(SCRIPT_DIR, "model", "gesture_model_4sec.pkl")
SCALER_PATH = os.path.join(SCRIPT_DIR, "model", "gesture_scaler_4sec.pkl")
...
def execute_command(gesture):
    ...
    if gesture == 'up': keyboard.tap(Key.media_volume_up)
    elif gesture == 'down': keyboard.tap(Key.media_volume_down)
    elif gesture == 'right': keyboard.tap(Key.media_next)
    elif gesture == 'left': keyboard.tap(Key.media_previous)
            ...
            with serial.Serial(self.port, self.baud, timeout=0.1) as ser:
                while self.running:
                    ...
                    if m:
                        ...
                        with self.lock:
                            self.fusion.update(parsed[:3], [p * (math.pi/180.0) for p in parsed[3:6]])
...
def main():
    if not (os.path.exists(MODEL_PATH) and os.path.exists(SCALER_PATH)):
        print("Error: 4sec model not found. Run train_model_4sec.py first.")
        return
    ...
    print("\n--- 4s PEAK-ENERGY DETECT STARTING ---")
    try:
        while True:
            with reader.lock:
            ...
            dynamic_threshold = max(baseline_rms * 1.5, 0.01)
            is_moving = rms > dynamic_threshold

            if system_status == "CALIBRATING":
                calibration_samples.append(rms)
                print(f"\rStep 1: CALIBRATING... {len(calibration_samples)}/100 (RMS: {rms:.4f}g)", end='', flush=True)
            ...
            if system_status == "ARMED":
                ...
                if is_moving:
                    print("\n[CAPTURING] Recording 4s gesture...")
            ...
            if system_status == "CAPTURING":
                ...
                if duration >= WINDOW_DURATION:
                    ...
                    for col in range(9):
                        ...
                        # Detrend (essential to remove bias/always right)
                        detrended = axis_data - np.mean(axis_data)
                        # Stats + Physics
                        features.extend([np.mean(detrended), np.std(detrended), np.max(detrended), np.min(detrended)])
                        features.append(np.sum(detrended))
                        features.append(np.sum(detrended**2))
                        features.append(np.max(np.abs(detrended)))
                    ...
                    print(f"[COOLDOWN] Waiting {GAP_DURATION}s...")
                    cooldown_start_time = time.time()
                    system_status = "COOLDOWN"
                continue

            if system_status == "COOLDOWN":
                remaining = GAP_DURATION - (time.time() - cooldown_start_time)
                if remaining > 0:
                    print(f"\r[WAIT] Restarting in {remaining:.1f}s...         ", end='', flush=True)
                else:
                    print("\n[READY] Armed.")
                    system_status = "ARMED"
                continue
