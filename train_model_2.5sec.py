...
FS = 100.0  
WINDOW_DURATION = 2.5 # Reduced for speed
CUTOFF = 5.0
ORDER = 4
MADGWICK_BETA = 0.1
...
def main():
    ...
    for gesture in os.listdir(DATA_4SEC):
        ...
        for filename in os.listdir(gesture_path):
            if filename.endswith(".txt"):
                ...
                if feat:
                    X.append(feat); y.append(gesture)
                    
    # 2. Load idle from data/idle
    ...
    joblib.dump(model, os.path.join(MODEL_DIR, "gesture_model_2.5sec.pkl"))
    joblib.dump(scaler, os.path.join(MODEL_DIR, "gesture_scaler_2.5sec.pkl"))
    print("Model saved as model/gesture_model_2.5sec.pkl")
